...
- [x] **P1 — Run-engine spine.** Extend `lib/agent-sdk.ts:streamChat`; adopt the hermes-webui run-adapter event contract + turn-journal; add `runs`/`run_events` tables; live Skills tab (§6.8); artifact-generation path (§6.9). *(shipped 2026-05-18 — `feat/faro-p1-run-engine` → main; L4 validated end-to-end on pei)*
...
- [x] **P2 — Studio-as-surface.** Dockview workspace + projects table (old 4.6 Workstream A). Panels now include Run + Board, not just artifacts. *(SHIPPED 2026-05-18→19 — P2 `feat/faro-p2-studio-as-surface` → main `631bb2c`; P2.1 L4 bug-fix batch `fix/faro-p2.1-l4` → main `5d62fdb`; operator L4 validated on pei (the merge is the acceptance signal); deployed)*
- [x] **P3 — HIL gate unification.** `approval` / `clarify` as run primitives merged with the dreams/claims surfaces (old 4.6 + hermes-webui flow). Preserves partner mode. *(IMPLEMENTED 2026-05-19 — `feat/faro-p3-hil-gate-unification`; one `UnifiedGate` projection + `GateResult` envelope + a selective run-gate re-arm, stores NOT physically merged; L1–L3 + L5 green on laptop (L2 181/181 across 24 suites; e2e 13 passed/4 env-gated-skip incl. P2/dreams/nav/studio regressions intact; migrate idempotent ×2, shared `state.db` mtime unchanged). **Pending: operator L4 on pei + merge→main — not pushed/merged (partner mode).**)*
...
**2026-05-18 (iteration #7 — P2 Studio-as-Surface SHIPPED; plan `.agents/plans/faro-p2-studio-as-surface.md`):**

33. **P2 Studio-as-Surface — the 4 locked answers (acceptance round 2026-05-18) + 2 refinements, implemented, not redesigned:**
    - **(1) Projects = grouping key + path hardening.** New `projects` + `ui_state` tables, nullable `project_id` FK (`ON DELETE CASCADE`) on `runs`/`artifacts`, all additive and **mirrored byte-identically across `scripts/migrate.ts` ⇆ `lib/db.ts:ensureSchema`** (the recurring drift class — closed for P2 tables; the pre-existing `claim_decisions.promote_to`/`.tweak_patch` `ensureSchema` omission was also fixed here). Vendored (Apache-2.0, attribution-only, no upstream NOTICE) from `nexu-io/open-design@34f66113a0f2391714d081d848e7dc48a5222de0`: `validateProjectPath`/`isSafeId`/`resolveExistingPrefix` verbatim + `assertUnderReal` composing the UNCHANGED lexical `lib/security.ts:assertUnder` for descendant-symlink defense. Soft-archive only (a hard project delete would cascade `runs` index rows that `reconcileRunsFromJournals` resurrects from the journals sans `project_id` — journal is source of truth, P1 Q2).
    - **(2) Panels:** sticky non-closable Home + Gallery + read-only Board by default; Artifact + Run open on-demand with deterministic-id dedupe (`run:<id>` / `artifact:<id>` → focus, not duplicate). `ProvenanceTabs` stays a tab-pair INSIDE the Artifact panel (not split).
    - **(3) Additive client-shell mount.** `dockview@^6.3.0` (resolved 6.3.0; import `dockview`, React-19 peer OK, no `overrides`). RSC `studio/page.tsx` resolves all server data + RSC nodes and renders them through a thin `"use client"` `StudioShellClient` wrapper that does the `next/dynamic({ssr:false})` (Next 16 forbids `ssr:false` in a Server Component — the plan's literal inline snippet was adjusted to the documented client-wrapper pattern, same architecture). Existing `Gallery`/`StudioWorkspace`/`ProvenanceTabs`/`Chat`/`RunView` reused **verbatim** as panel contents. **`renderer:'always'` MANDATE** on Run/Artifact panels (Dockview default `onlyWhenVisible` would unmount inactive panels and tear down RunView's SSE / the 4.5 Chat stream / the highlight-bridge iframe — the #1 regression risk).
    - **(4) Board** is read-only over the P1 `runs` query (`SELECT run_id,status,skill_name,created_at,cost_usd … ORDER BY created_at DESC LIMIT 100`); UTC-normalized timestamps (the documented 4.5 SQLite-TIMESTAMP gotcha). No drag, no dispatch.
    - **Refinements (consistent with locked intent, not scope changes):** profile-scoped layout lands in the new `ui_state(profile_id PK, …)` table (no profile SQLite table exists — profiles are YAML) wrapped in a `{schema,dockviewBuild,layout}` envelope with discard-on-mismatch (Dockview gives no `SerializedDockview` cross-version guarantee); CSP unchanged (`proxy.ts` already ships `style-src 'self' 'unsafe-inline'`, covering Dockview's runtime inline-style writes). Single active Artifact panel bound to the route `artifactId` (per-artifact RSC `Provenance`/`CodeRenderer` can't multiplex in a client Dockview panel without re-architecting 4.5; artifact switch = navigate + layout restore + rebind) — full multi-artifact = **P3+**. ⌘P quick-switcher (vendored `scoreMatch`/`nextCursor`, server union via `/api/switch`, `cmdk` + `shouldFilter={false}` so server ranking is authoritative, separate CustomEvent from ⌘K, browser-print suppressed).
    - **Execution-discovered fixes (caught by the test/e2e gates, not in the plan):** the `idx_artifacts_project` index in `db.ts:ensureSchema` must be guarded by a table-exists check (ensureSchema deliberately does not create `artifacts`, so a bare `CREATE INDEX … ON artifacts` threw `no such table` on a fresh lazy-open DB — unit tests caught it before it could ship); the e2e guard runs against the **dev** server (`proxy.ts` auth-gates loopback in production; dev injects the owner login) on the default `localhost` origin (Next 16 dev blocks `127.0.0.1` cross-origin by default).
    - **Out of P2 (held):** popout/floating groups, full Kanban dispatch, HIL gate *unification* (P3), sniper/handoff/exports (P4). The deep "4.5 Chat survives inside a Dockview panel across a tab switch" e2e (the `renderer:'always'` proof) is a seeded-fixture/manual-L4 check (plan-sanctioned degradation; same convention as `e2e/dreams-regression.spec.ts`). `/runs/[runId]` route survives P2 unchanged.
    - **Validation:** L1 (lint 0 errors — 1 pre-existing unrelated `ImageRenderer` `<img>` warning, not P2 — + types 0), L2 **159/159 tests** (139 prior + 20 new across 5 P2 suites incl. the `assertUnderReal` symlink-escape proof), L3 standalone build OK + e2e 6 passed/1 expected-skip on localhost, L5 migrate idempotent ×2 — all green on the laptop; Level 4 = pei live test pending. DB isolation held throughout (every migrate/test ran with `FARO_STATE_DB=/tmp/…`; the shared `slack_agent/runs/state.db` mtime stayed unchanged, verified).

34. **P2.1 — L4 bug-fix batch (operator L4 findings on pei, 2026-05-18; no scope re-decision, two knowing single-user overrides):**
    - **Bug 1+2+4 — layout + single Artifact panel.** Locked layout (operator-chosen): LEFT = sticky non-closable Home + Gallery in one `group.locked` group; RIGHT (big "work" group) = Board + Run + the single Artifact panel. `openPanel`/`syncArtifactPanel` route Artifact/Run to the work group via `targetFor()` (was: whatever `activeGroup` happened to be → artifacts opened in the narrow Gallery pane, bug #2). The N `artifact:<id>` panels sharing one `activeArtifact` prop (ghost "no artifact selected" tabs, bug #4) are replaced by ONE canonical `artifact:single` panel: `updateParameters()` re-renders it against the route artifact on navigation; closed when the route has none. This is the *locked* P2 single-artifact scope, finally implemented faithfully.
    - **Bug 3 — minimal project surface** (locked "minimal now, rich UI = P3"): new `GET /api/projects` + `GET /api/projects/[id]` (its runs+artifacts), a `ProjectPanel`, a clickable project list on Home, a shell `project` panel kind + `faro:open-project` CustomEvent. ⌘P project-pick now opens that panel (was a silent `router.push("/studio")` no-op); creating a project opens it immediately. No global active-project filter (deferred to P3).
    - **Bug 5 — permissive tool policy (KNOWING override of P1 Q1 + Q3/N1, single-user authorized).** New `lib/tool-policy.ts`: studio Chat (`streamChat`, 4.5-frozen surface) gets a read/search `allowedTools` + mutation/exec `disallowedTools` + `permissionMode:"default"` + `maxTurns` raised from 1 (so a tool turn can be followed by the answer) — it was dead-ending on an interactive permission prompt. Run-engine **agent** mode flips default-deny → default-ALLOW, hard-denying only destructive shell (`isDangerousBash`). The HIL gate/`resolveGate`/`awaiting_approval` machinery is KEPT but dormant (one-flip re-enable). `ensureOAuthAuth`/`pathToClaudeCodeExecutable` untouched (billing-leak rule holds).
    - **Bug 6 — Skills usage from faro runs.** `getSkillUsage` repointed off the vendored `~/.claude` JSONL→`turns` ingest (which never sees faro runs) to faro's own `runs` table by `skill_name` (7d count + last-used). Upstream ingest apparatus retained (exported + still unit-tested) for future correlation; documented divergence.
    - **Bug 7 — Generate on project skills (KNOWING override of P1 Q4 imported-only).** Three scope filters (`SkillCard.tsx`, `runs/page.tsx`, `api/runs/route.ts`) broadened to accept `scope==="project"`; parser/loadSkillBody were already scope-agnostic.
    - **Validation:** L1 lint 0/types 0; L2 **164/164** (5 P2 suites + new `lib/tool-policy.test.ts` + `usage.test.ts` rewritten to the faro-runs source + the 2 P1 gate tests rewritten to the permissive contract); L3 standalone build OK + e2e 6 passed/1 expected-skip on localhost; L5 migrate idempotent ×2 (P2.1 adds NO schema). DB isolation held (shared `state.db` mtime unchanged across all runs). Two pre-existing, non-fatal, **out-of-P2.1-scope** signals surfaced by the e2e and left for a future pass: `revalidatePath`-during-render in `lib/artifacts.ts:255` (scanArtifacts), and a hydration mismatch in the `TopLocator` chrome pill.

**2026-05-19 — P2 LINE SHIPPED & MERGED (closure, mirrors the P1 2026-05-18 ship):** P2 (`feat/faro-p2-studio-as-surface` `3b65d5d` → main `631bb2c`) and the **P2.1 L4 bug-fix batch** (`fix/faro-p2.1-l4` `ad2c809` → main `5d62fdb`) are both merged to `main`. The operator ran the Level-4 functional test on pei across P2 + the P2.1 fixes and **accepted — the merge of `fix/faro-p2.1-l4` IS the L4 acceptance signal** (same convention as P1's 2026-05-18 "origin/main fast-forwarded → pei back on main" closure). pei tracks `main` again (cron + deploys track main; the per-phase feature-branch L4 dance is retired, as it was for P1). The two deferred non-fatal signals above (`revalidatePath`-in-render, `TopLocator` hydration) are the only open follow-ups and are explicitly **out of the P2 line**. **P2 deliverable complete; roadmap advances to P3 (HIL gate unification) / P4 (sniper + handoff).**

**2026-05-19 (iteration #8 — P3 HIL gate unification IMPLEMENTED; plan `.agents/plans/faro-p3-hil-gate-unification.md` + its locked VALIDATION ADDENDUM):**

35. **P3 HIL gate unification — the 4 locked answers + the operator-signed forks (acceptance round 2026-05-19), implemented, not redesigned:**
    - **Stores stay separate; only the resolve contract + surface unify.** New `lib/gate-inbox.ts` projects BOTH sources into one `UnifiedGate` (discriminated on `source: "run" | "claim"`) and a hermes-`ControlResult`-shaped `GateResult` envelope; `resolveUnifiedGate` dispatches run gates INLINE to the UNCHANGED P1 `resolveGate`/`/api/runs/[runId]/answer`, and claim gates are NAVIGATIONAL only (deep-link to the unchanged sharded `/dreams/<runId>`; the Inbox NEVER stamps `claim_decisions`). No new SQLite table/index; no Python/Slack change; `slack_agent/pipeline_approvals.py` + `app/actions/claims.ts` + `lib/dreams.ts` byte-untouched (Slack remains the sole dream fallback).
    - **Selective re-arm, not blanket default-deny (partially reverts decision 34's permissive flip).** `lib/tool-policy.ts` gains a third `{behavior:"gate"}` `ToolDecision`; `GATE_TOOLS = [Write,Edit,MultiEdit,NotebookEdit]` + an **operator-signed conservative `isGatedBash`** pattern set gate mutating actions; read/search auto-allow; destructive shell stays a hard DENY (never gated, FIRST); the 4.5 Chat `CHAT_*` lists are byte-unchanged (P1 Q1 still holds). The run-engine `canUseTool` journals an `approval` + denies-WITHOUT-interrupt (verified @`claude-agent-sdk` 0.2.140), making the dormant P1 gate LIVE again, narrowly.
    - **Approved-set replay (highest-risk item — the one piece hermes solves with `approve_session`).** On a resumed turn `streamRun` rebuilds an approved-set FROM THE JOURNAL (`approvalKey = tool + " " + JSON.stringify(args)`, paired non-deny `gate_resolved`→originating `approval`) and short-circuits `canUseTool` to allow before `agentToolDecision` — so an approved tool is not re-gated into an infinite loop. Journal-derived (P1 Q2), surviving `consumeRun` re-entry + a faro restart between gate and answer; not an in-memory map.
    - **Operator-signed forks (P3 acceptance round, 4 AskUserQuestion answers):** (1) the literal conservative `GATE_TOOLS`/`isGatedBash` set; (2) the `approvalKey` shape as-is (documented JSON-key-ordering caveat); (3) the claim projection lifts the existing `/dreams` queue VERBATIM and is **deliberately NOT profile-scoped** (it agrees with the unscoped source-of-truth queue; `pipeline_runs`/`claim_decisions` carry no usable `profile_id`; run side stays profile-scoped — a recorded asymmetry that overrides the generic "profile-scope every read" for the claim branch only); (4) a lightweight env-gated e2e seed (deep approve-resume = manual-L4, plan-sanctioned).
    - **Refinements (consistent with locked intent, NOT scope changes; resolved during execution):** (a) a fresh per-turn `gateEmittedThisTurn` flag pauses a gated run at `awaiting_approval` instead of finalizing `done` on the post-deny `result` — the dead legacy `gatePending` const/block left untouched (out of scope); (b) the claim projection degrades to run-gates-only (tableExists guard + try/catch) rather than 500 the whole Inbox when `pipeline_runs` is absent or the `scripts/migrate.ts` stub lacks `created_at`/`result_summary` (prod's slack_agent table has them; `migrate.ts` NOT modified — recurring-drift class, out of scope); (c) `ensureGateInbox` makes the sticky `renderer:'always'` Gate Inbox panel present on a RESTORED pre-P3 P2 layout too, not only `buildDefaultLayout` (operators with a saved layout still get the unified surface); (d) hermes attribution extended (existing P1 `5e518b1c` kept; P3 `GateResult`/approved-set lift pinned `71c70352…`; the Nous-Hermes `tools.approval` runtime explicitly NOT vendored — `LICENSE-third-party.md`).
    - **Validation:** L1 lint 0/types 0; L2 **181/181** (24 suites incl. new `lib/gate-inbox.test.ts`, extended `lib/tool-policy.test.ts`, the reconciled run-engine gate/approved-set/deny tests; the 2 P2.1-permissive P1 gate tests re-reconciled to the P3 selective contract); L3 standalone build OK + e2e **13 passed / 4 env-gated-skip** against the dev server (P2 `studio-dockview`/`studio`/`dreams`/`nav` regressions all green; gate-inbox HTTP + sticky-panel green; deep approve-resume + seeded render are the documented manual-L4 / env-gated path — same convention as `e2e/dreams-regression.spec.ts`); L5 migrate idempotent ×2 (P3 adds NO schema). DB isolation held — shared `slack_agent/runs/state.db` mtime byte-identical (`2026-05-13 22:13:20`) across every test/migrate/e2e run (all via isolated `FARO_STATE_DB`). The two pre-existing P2 deferred signals (`revalidatePath`-in-render, `TopLocator` hydration) remain explicitly **out of P3 scope**. **L4-on-pei + merge→main pending operator acceptance (partner mode — no push/merge without approval).**
